...
const int max = 8;
const bool fich = false; // crear mazo de manera aleatoria (false) o cargarlo a partir de un fichero indicado (true)
const bool dep = false; // muestra en pantalla el contenido del mazo de cartas después de crearlo y tras cualquier modificación en él (true) o no (false)
const int MAX = 40;
...
// funciones modo A
float modoA(ifstream& archivo, int numCartas); //  recibe el archivo con el mazo y el numero de cartas a robar, calcula y devuelve la puntuación

//funciones modo B
float modoBhumano(ifstream& archivo, int numCartas); // recibe el archivo con el mazo y el maximo numero de cartas a robar y juega el humano hasta que desee parar o se pase de 7.5. Devuelve la puntuación del jugador
float modoBmaquina(ifstream& archivo, int numCartas, float puntJugador); //// recibe el archivo con el mazo y el maximo numero de cartas a robar y roba cartas mientras su puntuación sea menor que la del jugador y que 7.5. Devuelve los puntos de la máquina.
...
void vaciarArray(tCartasPorAparecer cartas, int carta); // modifica el array que recibe según la carta que se haya robado
bool esProbablePasarse(float puntosMaquina, const tCartasPorAparecer cartas); // recibe los puntos de la máquina y el array de cartas que quedan en el mazo y calcula la probabilidad de pasarse de 7.5 robando una carta. Devuelve true si la probabilidad supera el 0.5 y false en caso contrario.

//funciones modo C
void modoChumano(ifstream& archivo, tCartasPorAparecer cartas, float& puntos); //  recibe el archivo con el mazo, el array que indica cuantas cartas hay de cada. Permite al jugador robar las cartas que quiera y se calcula su puntuación, la cual se guarda en la variable por referencia. Se vacía el array de cartas cada vez que se roba.
void modoCmaquina(ifstream& archivo, tCartasPorAparecer cartas, float puntosHumano, float& puntos);  //  recibe el archivo con el mazo, el array que indica cuantas cartas hay de cada. La máquina roba cartas mientras su puntuación sea menor que la del jugadoy y que 7.5 y que la probabilidad de pasarse no supere 0.5 (en caso de empate). A su vez se calcula su puntuación, la cual se guarda en la variable por referencia. Se vacía el array de cartas cada vez que se roba.

// funciones modo D
void inicializa(tConjuntoCartas & cartas); //crea el mazo ordenado, aumenta el contador por cada carta que se añade y lo guarda en el struct
...
void annadirCarta(tConjuntoCartas & cartas, int carta); // añade  la carta robada a un struct que guarda las cartas del jugador o de la maquina y aumenta su contador
