...
public:
	...
	//dado un elemento e devuelve si existen en el conjunto elementos mayores o iguales que e y en
	//caso afirmativo cual es el menor de ellos.
	std::pair<bool, T> lower_bound(T const& e) const {
		auto it = begin();
		while (it != end() && *it < e)
			++it;
		return (it == end()) ? std::pair<bool, T>(false, 0) : std::pair<bool, T>(true, *it);
	}
	...
	int count_interval(const T &lower, const T &upper) const {
		...
		auto it = begin();
		while (it != end()) {
			if (lower <= *it && *it <= upper) ++n;

			if (*it <= lower) it = it->dr;
			else if (*it >= upper) it = it->iz;
			else {
				++it;
			}
		}
